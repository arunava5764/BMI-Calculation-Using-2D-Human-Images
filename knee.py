...
staff=image[start1:newend,:]
yaxisleft=[]
yaxisright=[]

for i in range(start1,newend+1):    
    for j in range(300):
        if image[i][j]==255:
            yaxisleft.append(j)
            break
...
xaxis=np.arange(start1,newend+1).reshape(((end1-start1))+1)
distances=np.array(distances).reshape(((end1-start1))+1)
# =============================================================================
